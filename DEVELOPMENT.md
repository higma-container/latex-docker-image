...
このドキュメントは、`latex-docker-image` の開発・リリース時に使用するメモです。
...
```sh
docker run --rm latex:local latex --version
```
...
docker buildx build \
  ...
  -t ghcr.io/higma-container/latex:v0.8 \
  --push .
```
...
```text
ghcr.io/higma-container/latex:v0.8
...
```yaml
cache-from: type=gha,scope=latex-amd64
cache-to: type=gha,mode=max,scope=latex-amd64
```

arm64：

```yaml
cache-from: type=gha,scope=latex-arm64
cache-to: type=gha,mode=max,scope=latex-arm64
```
...
test.yaml
   │
   ├── amd64 ──→ latex-amd64 cache
   │
   └── arm64 ──→ latex-arm64 cache
                         │
                         ↓
                    release.yaml
```
...
```text
higma-container/latex-docker-image
```
...
```dockerfile
LABEL org.opencontainers.image.source="https://github.com/higma-container/latex-docker-image"
```
